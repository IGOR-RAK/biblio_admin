...
const CardsItem:FC <ICardItem> = ({card}) =>{
    const {cards,store,years} = React.useContext(Context); 
    ...
    return (
              <div key={card._id} className='row mb-3'>
                ...
                <div className='col-sm-6 '>           
                  ...
                  {card.link&&<div><a href={card.link} target="_blank" rel="noreferrer">Link</a></div> } 
                  <div className='btn-group'>
                    <button className='btn btn-secondary' onClick={()=>{handleEdit(card)}}>Edit</button>                   
